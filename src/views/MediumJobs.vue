<template>
  <div>
    <div class="page-header"><h1 class="page-title"><span>👌</span> 中等工作</h1><button v-if="appStore.isLoggedIn" class="btn btn-warning" @click="showForm = true">+ 添加记录</button></div>
    ...
    <div v-else-if="appStore.jobs.medium.length === 0" class="empty-state">
      <div class="empty-state-icon">📋</div><div class="empty-state-text">还没有中等工作记录</div>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitJob">
          ...
          <div style="display:flex;gap:8px;justify-content:flex-end">
            ...
            <button type="submit" class="btn btn-warning" :disabled="publishing||aiChecking">{{ publishing?'发布中...':aiChecking?'校验中...':'添加记录' }}</button>
          </div>
        </form>
      </div>
    </div>
